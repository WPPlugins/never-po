		  ...
		  if ( node ) {
			...
			do {
			  ...
			  if ( node.nodeType === 3 && $(node).parent()[0].tagName == target_tag ) {
				...
				// The original node value.
				val		= node.nodeValue.match(search)[0];	//строка очищается regexp
				new_val = replace(val);						//получаем контенер с данными об объекте перевода
				...
				if ( typeof new_val.translated_text != 'undefined' && new_val.translated_text !== val ) {
